...
gulp.task('scripts', function() {
	return gulp.src([
		...
		'assets/js/_libs.js', // JS libraries (all in one)
...
gulp.task('img-responsive-1x', async function() {
	return gulp.src('assets/img/_src/**/*.{png,jpg,jpeg,webp,raw}')
		.pipe(newer('assets/img/@1x'))
		.pipe(responsive({
			'**/*': { width: '50%', quality: quality }
		})).on('error', function (e) { console.log(e) })
		.pipe(rename(function (path) {path.extname = path.extname.replace('jpeg', 'jpg')}))
		.pipe(gulp.dest('assets/img/@1x'))
});
// Produce @2x images
gulp.task('img-responsive-2x', async function() {
	return gulp.src('assets/img/_src/**/*.{png,jpg,jpeg,webp,raw}')
		.pipe(newer('assets/img/@2x'))
		.pipe(responsive({
			'**/*': { width: '100%', quality: quality }
		})).on('error', function (e) { console.log(e) })
		.pipe(rename(function (path) {path.extname = path.extname.replace('jpeg', 'jpg')}))
		.pipe(gulp.dest('assets/img/@2x'))
});
...
gulp.task('watch', function() {
	...
	gulp.watch(['assets/js/_custom.js', 'assets/js/_libs.js'], gulp.parallel('scripts'));
	gulp.watch('assets/*.html', gulp.parallel('code'));
	gulp.watch('assets/img/_src/**/*', gulp.parallel('img'));
});
